...
function generateCsv(reports: DailyReport[]): string {
  const headers = [
    ...
    "IB_【500円引き】一般（休日）枚数",
    "IB_【500円引き】一般（休日）金額",
    "IB_【500円引き】こども（平日）枚数",
    "IB_【500円引き】こども（平日）金額",
    "IB_【500円引き】こども（休日）枚数",
    "IB_【500円引き】こども（休日）金額",
    "IB_【500円引き】一般VIP（平日）枚数",
    "IB_【500円引き】一般VIP（平日）金額",
    "IB_【500円引き】一般VIP（休日）枚数",
    "IB_【500円引き】一般VIP（休日）金額",
    "IB_【500円引き】こどもVIP（平日）枚数",
    "IB_【500円引き】こどもVIP（平日）金額",
    "IB_【500円引き】こどもVIP（休日）枚数",
    "IB_【500円引き】こどもVIP（休日）金額",
    "IB_貸切VIP枚数",
    "IB_貸切VIP金額",
    "IB_合計枚数",
    "IB_合計金額",
  ...
  const rows = reports.map((r) => {
    ...
    return [
      ...
      ib?.genWeekdayDiscount500.amount  ?? "",
      ib?.genHolidayDiscount500.count   ?? "",
      ib?.genHolidayDiscount500.amount  ?? "",
      ib?.childWeekdayDiscount500.count  ?? "",
      ib?.childWeekdayDiscount500.amount ?? "",
      ib?.childHolidayDiscount500.count  ?? "",
      ib?.childHolidayDiscount500.amount ?? "",
      ib?.genVipWeekdayDiscount500.count   ?? "",
      ib?.genVipWeekdayDiscount500.amount  ?? "",
      ib?.genVipHolidayDiscount500.count   ?? "",
      ib?.genVipHolidayDiscount500.amount  ?? "",
      ib?.childVipWeekdayDiscount500.count  ?? "",
      ib?.childVipWeekdayDiscount500.amount ?? "",
      ib?.childVipHolidayDiscount500.count  ?? "",
      ib?.childVipHolidayDiscount500.amount ?? "",
      ib?.vip.count  ?? "",
      ib?.vip.amount ?? "",
      ibRaw?.totalCount  ?? "",
      ibRaw?.totalAmount ?? "",
